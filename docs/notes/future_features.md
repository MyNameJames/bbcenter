...
| 15 | Workflow formalize — booking state machine กลาง | Vehicle | จาก workflow review 2026-06-11 (ตาราง transition จริง → architecture.md §Vehicle Booking Status Flow): (a) สร้าง `views/vehicle/vehicle_workflow.py` นิยาม ALLOWED_TRANSITIONS + `transition(booking, to, actor)` รวม guard/notify ที่เดียว (b) ยุบ approve/reject ที่ซ้ำใน `admin_assign` ให้ใช้ชุดเดียวกับ `approve_booking` (c) ~~ปิด 3 gaps~~ **ปิดแล้ว 2026-06-12:** `budget_manage` action เปลี่ยน `refund_booking`→`cancel_booking` · `admin_revert_booking` มี guard deduct + source status restriction; ยังเหลือ: `admin_assign` approve ไม่เช็กงบ active (d) เขียน test booking lifecycle **ก่อน** ลงมือ (e) ควบรวมงานเดิม: แตก budget_manage god function + decorator `@require_vehicle_admin` | 2026-06-11 |
