...
| `vehicle_cost.html` — feature redesign (นอก scope 6 จุด, เปลี่ยนโครงหน้าทั้งหมด) | 2026-08-08 | Section tabs แทน status-filter tabs เดิม + tab "ใบจ่ายจริง" ทำจริง (2-pane, endpoint `GET /admin/ot/slip` ใหม่) + tab "ตั้งค่า OT" ทำจริง (แทน `rateConfigModal`, ลบผ่านถังขยะ+confirm modal, มือถือเป็น card) + tab "ผู้ใช้จ่ายเอง" ทำจริง (list ไม่มี filter, layout อ้างอิง `vehicle_mileage.html`) + tab "ภาพรวมทั้งปี" ทำจริง (pivot คนขับ×เดือน สไตล์ `pivotMockupTable` ของ `vehicle_budget.html`, ย้ายมาจาก `#otPivotCard` เดิม + แก้ orphan class `.vc-pivot-td-sticky` ที่ไม่เคยทำงานจริงไปด้วย) + tab "ทั้งหมด" ทำใหม่ทั้งยวง (ตัด KPI/แถบอัตรา OT/ตารางแยกประเภทงานออก เหลือแค่ตาราง OT ไม่มี filter ล้วนๆ + ปุ่ม Excel — ลบฟังก์ชัน backend ที่ตายแล้ว 3 ตัวทิ้ง) — **ครบ 5/5 tab แล้ว** — รายละเอียดเต็มอยู่ [INDEX_ui.md](INDEX_ui.md) แถว `vehicle_cost.html` (ไม่ซ้ำที่นี่ เพราะไม่ใช่ประเด็น design-language 6 จุดของไฟล์นี้ แต่เป็น feature ใหม่) |
| `vehicle_admin.html` | — | ยังไม่ apply |
| `vehicle_budget.html` | — | ยังไม่ apply |

**Backlog (`vehicle_cost.html`, ตัดสินใจข้ามรอบนี้ ไม่ใช่ลืม):**
- 4 modal (แก้ไข OT / เพิ่ม OT / อัตรา OT / preview ใบเสร็จ) ยังเป็น Bootstrap default (`.modal-header`+X) ไม่ใช่ pattern A (§1.1) — งานใหญ่ รอคิวแยก. **`rateConfigModal` (อัตรา OT) ตอนนี้เป็น dead markup แล้ว** (แท็บ "ตั้งค่า OT" แทนที่ ไม่มีปุ่มเปิด) → ควร**ลบทิ้ง**พร้อม `buildRateRow()`/`addRateBtn`/`rateBox` handler ใน `vehicle_ot.js` และ `.cost-rate-*` ที่เหลือ แทนที่จะ restyle
- ลบช่วงอัตรา OT ในแท็บ "ตั้งค่า OT" = soft-delete ถาวร ไม่มี UI เปิดกลับ (ตัดสินใจ 2026-08-08 — ลองทำ toggle `is_active` ไปรอบหนึ่งแล้วเปลี่ยนใจกลับเป็นลบทิ้งแบบเดิมในวันเดียวกัน) ถ้าจะเอาคืนต้องแก้ DB มือ เหมือน gap เดิมของ `rateConfigModal`
