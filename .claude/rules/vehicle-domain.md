...
- **OT formula (2026-08-07, v2.30):** เงิน OT คิดที่ `domain/vehicle/ot.py::build_slot()` **ที่เดียวทั้งระบบ** — ห้าม inline สูตร. หน่วยของ rate อยู่ที่ `OTRateConfig.rate_type`: `hourly` = คูณนาทีจริง/60 · `flat_day` = เหมาเต็มจำนวนไม่คูณเวลา และคิด **ครั้งเดียวต่อ (คนขับ, วัน, band)** — ทุก path ที่สร้าง slot ต้องส่ง `claimed_flat_ids` จาก `mileage_service.claimed_flat_configs()` (auto/recompute/manual ครบทั้ง 3). ⚠️ `day_of_week` = "วันนั้นใช้ชุดอัตราไหน" **ไม่ใช่สัญญาณเหมาจ่าย** (เคยตีความผิดจนคิดเงินคนละสูตร 2 path) · band ข้ามเที่ยงคืน (`end < start`) ใช้ไม่ได้ ต้องแยก 2 ท่อน (ระบบห้ามจองข้ามวันอยู่แล้ว)
