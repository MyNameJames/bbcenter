...
| 17 | Booking trip card — mileage entry + OT + cost confirm | ต่อยอด **Case 12 + Case 9** รวมเป็นการ์ดเดียว กรอกเลขไมล์+ดูค่าใช้จ่ายจบในหน้าเดียว ไม่ต้องเปิด modal แยกแบบ Case 9 เดิม — ขยาย `bb-timeline` จาก 1 stop (Case 12) เป็น 3 stop: **รายละเอียด** (title คงที่ ไม่ผูกชื่อคนขับแล้ว, รถ/คนขับ+โทร/งบ แยกคนละบรรทัด, กล่องท้ายเปลี่ยนจาก "ประเภทงบ+ปุ่มแก้ไข" เดิม เป็น 2 ช่อง input เลขไมล์ยกจาก **Case 9** ทั้งชุด) → **ช่วงเวลา/ค่า OT** (บรรทัดเวลา+OT format ตามเงื่อนไข `<1ชม.`=นาที / `≥1ชม.`=ทศนิยมชั่วโมง — ต่างจาก Case 11 ที่โชว์ทศนิยมเสมอ, กล่องมี rate+ยอดรวม OT + เส้นเทียบเวลาปกติ(เทา `--bb-n200`)/OT(ส้ม `--bb-wr`) เป็น 2 `div` flex กว้างตาม % จริงไม่มี JS พร้อม label เวลา 3 จุด) → **ค่าใช้จ่ายทั้งหมด** (ระยะทาง/ราคาน้ำมันเป็นบรรทัด สอดคล้องสูตร `calc_fuel_cost` เดิม, กล่องรวมค่าน้ำมัน+OT แล้วยอดรวม**ตัวหนา**แยกบรรทัด). ท้ายการ์ด (นอก timeline) ปุ่ม ยกเลิก(`is-sec`)/ยืนยัน(`is-pri`) ชิดขวา — โครงจาก footer ของ Case 9 เปลี่ยนคำ "บันทึก"→"ยืนยัน" | `.bb-timeline` / `.bb-field` (Case 9) / เส้นเทียบเวลา 2 `div` flex (ไม่มี lib) | 1569–1789 |
